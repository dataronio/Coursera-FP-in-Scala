 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of some zeros") {
    assert(sum(List(0,0,0)) === 0)
  }
  test("sum of an empty list") {
    assert(sum(List()) === 0)
  }
  test("sum of a few numbers some repeated") {
    assert(sum(List(2,2,0,0)) === 4)
  }
  ...
  test("max of some zeros") {
    assert(max(List(0, 0, 0)) === 0)
  }
  test("max of a few numbers some repeated") {
    assert(max(List(2, 2, 0,0)) === 2)
  }
